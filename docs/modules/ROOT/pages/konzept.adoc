...
In diesem Konzept werden die Designvorgaben für die Überwachung von Anwendungen gemäß der xref:isyfact-standards-doku:referenzarchitektur:index.adoc[] vorgestellt.
...
Details hierzu finden sich in den xref:nutzungsvorgaben.adoc#vorgaben-fuer-die-pruefung-der-verfuegbarkeit[Nutzungsvorgaben].

[IMPORTANT]
Hinweis: Nachbarsystemchecks sollen ab der IF6 nicht mehr verwendet werden.
Für Neuentwicklungen wird bereits jetzt davon abgeraten, Nachbarsystemchecks neu zu implementieren.
Bestehende Implementierungen können bis zur Ablösung weiter betrieben werden.
...
Die betrieblich begründete, temporäre Nichtverfügbarkeit einer Anwendung, kann über die Löschung einer `isAlive`-Datei gesteuert werden (vgl. xref:nutzungsvorgaben.adoc#integration-des-loadbalancer-servlets[Nutzungsvorgaben]).
...
Die Nutzungsvorgaben finden sich in den xref:nutzungsvorgaben.adoc[].
