...
Alle Vorgaben orientieren sich an den jeweiligen Kapiteln aufgeführten Anforderungen und gelten für alle Anwendungen nach IsyFact-Architektur.
...
So werden für Informationen über den Systemzustand der Actuator-Endpunkt `health` sowie die darunterliegenden Health-Gruppen `liveness` und `readiness` angeboten.
Die Gruppen sind unter `/actuator/health/liveness` und `/actuator/health/readiness` erreichbar.
Über den Endpoint `metrics` können Metriken gelesen werden.
Die bereitgestellten Metriken werden von Micrometer erfasst.
Eine nähere Beschreibung des Konzeptes findet sich im Dokument xref:konzept.adoc#einleitung[Überwachung - Konzept].
...
Weiterhin können die hier gemachten Vorgaben als Basis für die Beschreibung der Überwachungsschnittstelle im xref:isyfact-standards-doku:methodik:vorlagen.adoc#dokumentation-anwendung[Systemhandbuch] verwendet werden.
...
* Health ist unauthentifiziert erreichbar.
* Andere Actuator-Endpunkte benötigen ein gültiges `JWT` mit Rolle `ENDPOINT_ADMIN`.
...
Die `MeterRegistry` wird von Spring per Dependency Injection bereitgestellt.
...
Dazu sind neben der Einhaltung der in dem Dokument xref:konzept.adoc#einleitung[Überwachung - Konzept] beschriebenen Vorgaben noch einige grundlegende  Regeln zu beachten:
...
Insbesondere dürfen keine fachlichen Funktionen des xref:isyfact-standards-doku:referenzarchitektur:software-technisch/backend/anwendungskern.adoc[Anwendungskerns] aufgerufen werden.
...
Zur Überwachung einer Anwendung stehen die folgenden eingebauten Endpoints bereit:

* `health`,
* `metrics`,
* `info`.

Unterhalb des `health`-Endpoints stehen die Health-Gruppen `liveness` und `readiness` bereit.
...
----
isy.task.tasks.isyHealthTask-execute.host=
----
...
Da von Spring standardmäßig fast alle Endpoints aktiviert sind, werden zunächst alle Endpoints ausgeschaltet, und anschließend die Endpoints `health`, `metrics` und `info` explizit wieder aktiviert.
...
Standardmäßig wird der Zugriff auf veröffentlichte Actuator-Endpunkte durch OAuth2/JWT abgesichert, wenn die erforderlichen Spring-Security-Abhängigkeiten im Klassenpfad verfügbar sind und die `IsyActuatorSecurityAutoConfiguration` aktiv ist.

Für die Absicherung werden folgende Dependencies benötigt:

* `org.springframework.security:spring-security-config`
* `org.springframework.security:spring-security-web`

Der Health-Endpunkt einschließlich der Health-Gruppen `/actuator/health/liveness` und `/actuator/health/readiness` ist unauthentifiziert erreichbar.
Diese Endpunkte müssen für Kubernetes-Probes, Loadbalancer und vergleichbare Betriebsmechanismen ohne Authentifizierung abrufbar sein.

Andere veröffentlichte Actuator-Endpunkte, insbesondere `metrics` und `info`, benötigen ein gültiges JWT mit der Rolle `ENDPOINT_ADMIN`.

Ist in der Anwendung bereits eine passende OAuth2-/JWT-Konfiguration vorhanden, kann diese für die Absicherung der Monitoring-Endpunkte verwendet werden.
Existiert keine Standard-Absicherung oder soll gegen einen anderen OpenID-Connect-Zertifikats-Endpunkt validiert werden, muss das Property `isy.ueberwachung.security.jwk-set-uri` gesetzt werden.
...
[source,properties]
----
# OpenID-Connect-Zertifikats-Endpunkt zur Validierung der JWT-Signatur
isy.ueberwachung.security.jwk-set-uri=https://<keycloak-uri>:<port>/auth/realms/<realm>/protocol/openid-connect/certs
----

[WARNING]
====
Wird in einer Anwendung eine eigene `SecurityFilterChain` definiert, muss sichergestellt werden, dass die Actuator-Sicherheitsregeln nicht unbeabsichtigt überschrieben werden.

Insbesondere dürfen `metrics`, `info` und weitere veröffentlichte Actuator-Endpunkte nicht durch eine allgemeine `permitAll`-Regel öffentlich erreichbar werden.
====

Die Standardabsicherung der Actuator-Endpunkte kann per Konfiguration deaktiviert werden:
...
Dabei muss beachtet werden, dass über Actuator-Endpunkte sensible technische Informationen bereitgestellt werden können.
Wird die Standardabsicherung deaktiviert, muss die Anwendung oder die Betriebsumgebung eine gleichwertige Absicherung bereitstellen.
...
Der Baustein liefert einen Nachbarsystem-Indikator mit, der für alle konfigurierten xref:glossary::terms-definitions.adoc#nachbarsystem[Nachbarsysteme] die jeweils konfigurierte Health-URL abfragt.
Für nach IsyFact betriebene Nachbarsysteme soll bevorzugt der Readiness-Endpunkt `/actuator/health/readiness` verwendet werden.
Der Baustein liefert einen Nachbarsystem-Indikator mit, der den HealthEndpoint oder den Readiness-Zustand aller konfigurierten Nachbarsysteme abfragt.
Die Klasse `NachbarsystemIndicator` wird durch `isy-ueberwachung` als Health-Contributor mit dem Namen `isyNachbarsystem` registriert.
Dieser Contributor ist standardmäßig Teil der Readiness-Gruppe.
...
isy.ueberwachung.nachbarsysteme.beispielnachbar.healthendpoint=http(s)://example.com/actuator/health/readiness
...
* `healthendpoint`: Die URL, unter der der HealthEndpoint oder der Readiness-Zustand des Nachbarsystems zu erreichen ist
...
Zusätzlich liefert `isyNachbarsystem` den Status DOWN.
Da `isyNachbarsystem` standardmäßig Teil der Readiness-Gruppe ist, wird auch `/actuator/health/readiness` DOWN.
Nicht essenzielle Nachbarsysteme beeinflussen den Health-Status und die Readiness nicht und werden, falls sie nicht erreichbar sind, nur als Warning geloggt.
...
Wie in Kapitel xref:konzept.adoc#informationen-ueber-den-systemzustand[Informationen über den Systemzustand] beschrieben sind _Liveness_ und _Readiness_ Zustände, die einem feingranularen Monitoring dienen.

Anwendungen nach dem IsyFact Standard müssen, neben dem Health-Zustand, Informationen über diese Zustände zur Verfügung stellen.
Für die Traffic-Steuerung, Loadbalancer oder Service-Mesh-Policies ist der Endpunkt `/actuator/health/readiness` maßgeblich.
Der bestehende Endpunkt `/actuator/health` bleibt aus Gründen der Abwärtskompatibilität unverändert verfügbar und dient weiterhin der allgemeinen Systemzustandsanzeige.
Die Bibliothek `isy-ueberwachung` ergänzt die _Readiness_ um die zentrale IsyFact-Logik für essenzielle Nachbarsysteme.
...
Die Bibliothek stellt Liveness und Readiness standardmäßig über Spring Boot Health Groups bereit.
Standardmäßig gelten folgende Gruppen:
...
Der `readinessState` bildet den nativen Spring-Boot-Lifecycle der Anwendung ab und `isyNachbarsystem` ist der bereitgestellte Health-Contributor für konfigurierte Nachbarsysteme.
Essenzielle Nachbarsysteme beeinflussen dadurch standardmäßig `/actuator/health/readiness`.

Der bestehende Endpunkt `/actuator/health` bleibt unverändert erhalten.
Für die Traffic-Steuerung, Loadbalancer oder Service-Mesh-Policies ist `/actuator/health/readiness` maßgeblich.
Für Neustartentscheidungen ist `/actuator/health/liveness` zu verwenden.

Bestehende Anwendungen müssen nichts anpassen.
Anwendungen, die eigene Readiness-Gruppen konfigurieren, müssen die Default-Includes beibehalten.
...
Gemäß dem IsyFact Vorgaben zählen dazu alle als essenziell eingestuften Nachbarsysteme (Vgl. xref:konzept.adoc#nachbarsystemcheck-liveness-Readiness[Überwachungskonzept-Essenzielle Nachbarsysteme].)
...
Durch dieses Event wird der von Spring Boot verwaltete Availability-State der Anwendung verändert.
Dieser State wird über `livenessState` bzw. `readinessState` in den jeweiligen Health-Gruppen sichtbar.
Damit ist eine vorhandene Implementierung um eine Veränderung des jeweils relevanten Zustands erweiterbar.
...
Spring Boot Actuator bietet, über vorkonfigurierte `HealthIndicator`-Interfaces, die Integration der health-Informationen angebundener Systeme an.
...
Wird die _Readiness_ des Systems von der Erreichbarkeit weiterer essenzieller Nachbarsysteme beeinflusst, ist bei Verwendung der Bibliothek `ueberwachung`
kein eigenes Gruppieren dieser Prüflogik notwendig.
Die Bibliothek stellt dafür den `NachbarsystemIndicator` unter dem Bean-Namen `isyNachbarsystem` bereit, der zusammen mit `readinessState` in die Readiness-Gruppe eingebunden wird.
...
Zusätzliche anwendungsspezifische Checks können weiterhin in die Gruppen aufgenommen werden:
...
[source,properties]
----
management.endpoint.health.group.readiness.include=readinessState,isyNachbarsystem,<customCheck>
management.endpoint.health.group.liveness.include=livenessState,<customCheck>
----

Mit dieser Konfiguration in den `application.properties` werden alle in der Gruppe aufgeführten Indikatoren verknüpft.
Der schlechteste relevante Status der Gruppenmitglieder bestimmt den Gruppenstatus gemäß Spring-Boot-Statusaggregation.


[[logging-der-zustandsaenderung]]
===== Logging der Zustandsänderung

Änderungen des Liveness- oder Readiness-Zustands sind im Logging zu protokollieren.
Dabei ist die Zustandsänderung zu protokollieren, nicht jede einzelne Zustandsprüfung.

Das Logging muss erkennen lassen,

* welcher Zustand betroffen ist (`Liveness` oder `Readiness`),
* welcher neue Zustand eingetreten ist,
* welche Komponente oder welcher Indikator die Änderung ausgelöst hat,
* zu welchem Zeitpunkt die Änderung erkannt wurde.

Dieses Vorgehen reduziert das Protokollvolumen gegenüber dem Protokollieren jeder Health-Prüfung und erleichtert die Analyse von Verfügbarkeitsproblemen.

Für durch Spring Boot verwaltete Zustände sind die Ereignisse von `ApplicationAvailability` maßgeblich.
Änderungen an `LivenessState` und `ReadinessState` sollen über die entsprechenden Spring-Boot-Availability-Events ausgewertet werden.

Für zusätzliche Health-Indikatoren, insbesondere `isyNachbarsystem`, ist sicherzustellen, dass Statusänderungen nachvollziehbar sind.
Bei Nachbarsystemen muss protokolliert werden, wenn ein essenzielles Nachbarsystem nicht erreichbar ist und nach einer Störung wieder erreichbar wird.
Nicht essenzielle Nachbarsysteme sollen bei Nichterreichbarkeit nur als Warnung protokolliert werden.

Das Logging darf keine sensiblen Informationen enthalten.
...
Der Prometheus-Endpoint ist zusätzlich nicht automatisch exposed und muss bewusst freigeschaltet werden.
Der Prometheus-Endpunkt ist kein Ersatz für den `metrics`-Endpoint.
Er stellt Micrometer-Metriken im Prometheus-Scrape-Format bereit und muss betrieblich abgesichert werden, z. B. über Netzwerksegmentierung, Service Mesh oder die Actuator-Security-Konfiguration.
...
Das Loadbalancing-Servlet ist Teil der Bibliothek isy-ueberwachung. Es wird automatisch durch die Einbindung der Bibliothek als Servlet registriert.
Die zu verwendende isAlive-Datei kann über die Property
[source,properties]
----
isy.ueberwachung.loadbalancer.isAliveFileLocation
----
in der `application.properties` konfiguriert werden.

==== Verhalten bei Embedded Tomcat

Wird die Anwendung mit einem Embedded Tomcat betrieben (z. B. als Spring-Boot-Anwendung), wird der konfigurierte Pfad als absoluter Dateisystempfad interpretiert.

Beispiel:
[source,properties]
----
isy.ueberwachung.loadbalancer.isAliveFileLocation=/etc/<anwendungsname>/isAlive
----
In diesem Fall prüft das Loadbalancer-Servlet direkt die Existenz der Datei `/etc/<anwendung>/isAlive`. Der Standardpfad `DEFAULT_IS_ALIVE_FILE_LOCATION` wird für Embedded-Tomcat-Anwendungen nicht verwendet.

==== Verhalten bei externem Servlet-Container

Wird die Anwendung in einem externen Servlet-Container (Tomcat, nicht embedded) betrieben, wird der konfigurierte Pfad relativ zum Servlet-Kontext aufgelöst.

Beispiel:
[source,properties]
----
isy.ueberwachung.loadbalancer.isAliveFileLocation=config/isAlive
----
Der Pfad wird in diesem Fall über den Servlet-Kontext ermittelt und relativ zur Anwendung interpretiert.

==== Nutzung des Loadbalancing-Servlets

Die Verfügbarkeit der Anwendung kann über die URL
[text]
----
http(s)://<serverurl>/<anwendungsname>/Loadbalancer
----
abgefragt werden.

Für eine positive Statusmeldung muss die konfigurierte IsAlive-Datei vorhanden sein. Zur Steuerung des Loadbalancer-Servlets kann der Betrieb diese Datei anlegen oder entfernen. Das Anlegen bzw. Entfernen kann bei Bedarf über Shell-Skripte oder entsprechende Betriebswerkzeuge automatisiert werden.
