...
_Liveness_ und _Readiness_ sind konzeptionell beschrieben und werden durch Spring Boot bereitgestellt.
Die Bibliothek `isy-überwachung` ergänzt die _Readiness_ um die zentrale IsyFact-Logik für essenzielle Nachbarsysteme.
...
Ob Abhängigkeiten mit Einfluss auf die Zustände gemäß deren xref:konzept.adoc#Liveness-und-Readiness[Definition] bestehen, ist von den Entwickelnden zu beurteilen.
...
Wird die _Readiness_ des Systems von der Erreichbarkeit weiterer essenzieller Nachbarsysteme beeinflusst, ist bei Verwendung der Bibliothek `isy-überwachung`
