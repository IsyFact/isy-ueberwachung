...
Standardmäßig wird der Zugriff auf Actuator-Endpunkte durch OAuth2/JWT abgesichert, wenn die erforderlichen Spring-Security-Abhängigkeiten im Klassenpfad verfügbar sind und die `IsyActuatorSecurityAutoConfiguration` aktiv ist.
...
Wird eine eigene `SecurityFilterChain` definiert, muss sichergestellt werden, dass diese die Actuator-Sicherheitsregeln nicht unbeabsichtigt überschreibt.
Der Health-Endpunkt einschließlich der Health-Gruppen `/actuator/health/liveness` und `/actuator/health/readiness` ist unauthentifiziert erreichbar.
Andere veröffentlichte Actuator-Endpunkte, insbesondere `metrics` und `info`, benötigen ein gültiges JWT mit der Rolle `ENDPOINT_ADMIN`.
